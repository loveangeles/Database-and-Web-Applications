...
function handleStarResult(resultData) {
	...
	if(resultData.length != null){
		for (var i = (curr_page-1)*limit; i < Math.min(limit*curr_page, resultData.length); i++) {
			...
			var starHTML = "<td>";
			var x=0
			for (; x<star_array.length; ++x)
				starHTML += "<a href=/fabflix/SingleStar.html?name=" + encodeURIComponent(star_array[x]) + ">" + star_array[x] + "</a>, ";
			starHTML += "<a href=/fabflix/SingleStar.html?name=" + encodeURIComponent(star_array[x]) + ">" + star_array[x] + "</a>";
			rowHTML += starHTML + "</td>";
			rowHTML += "<td><input type=\"button\" onclick=\"window.location.href='/fabflix/ShoppingCart.html?title="+encodeURIComponent(resultData[i]["movie_title"])+"'\" value=\"Add to Cart\">";
			rowHTML += "</tr>";
